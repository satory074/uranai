...
- タロットの `<TarotCard />` はそれ自身が `<button>`。`flipped: boolean` と `onFlip?: () => void` を **props で受け取る controlled component** で、`flipped` の真偽は `Home.tsx` の `tarotFlipped: [boolean, boolean, boolean]` state がソース・オブ・トゥルース。クリックで `onFlip` が呼ばれて Home の state が立ち、再描画で `flipped=true` が降りてカードがめくれる (`disabled={flipped}` で再クリック不可)。`position?: string` (過去/現在/未来 の左上バッジ) と、正/逆で切り替わる `card.upright.keywords` / `card.reversed.keywords` の `・` 連結行を前面下部に表示する。`reversed` の場合は `.card-orient` に `data-reversed="true"` が付き、CSS が **静的に** `transform: rotateZ(180deg)` を当てて前面を 180° 回転させる (旧 `orient-reversed` キーフレームの「逡巡 → オーバーシュート」アニメは廃止、めくった後の最終状態だけが逆さま)。位置バッジは `.card-orient` の**外側**にあるので逆位置でも回転せず常に正向きで読める。`Home.tsx` の `TAROT_POSITIONS = ['過去','現在','未来'] as const` を `position` prop に渡す。
- タロットの **結果テキストはカード単位で gating** されている: Home が `tarotThree.result` から派生 `gatedTarotResult` を作り `<FortuneResultView>` に渡す。未めくりのセクションは `title` が `TAROT_POSITIONS[i]` のみ (例: 「過去」)、`body` が `'カードをタップして結果を見る'` のヒント文に置換され、めくれたセクションだけが engine 由来の本来の `title` (「過去 — 教皇（逆）」) と `body` (キーワード行 + 解釈) に切り替わる。`subtitle` (3 枚並列の "教皇逆 / 力 / 運命の輪") は **3 枚すべて** めくれるまで `undefined` に伏せて `<FortuneResultView>` の subtitle 表示と、`deriveOneLiner` 経由のページ上部 `<FortuneDigest>` のチップ文も `subtitle` 由来なので連動して伏せられる (`r.subtitle ?? r.title` のフォールバックで「過去・現在・未来 3枚スプレッド」が出る)。`headline` (キーワードチップ群) は `deriveHeadline` が `sections[0].body` から取るため、1 枚目がめくれるまで `undefined` を渡してチップ非表示。`tarotFlipped` のリセットはフォーム `onSubmit` で行う (`setTarotFlipped([false, false, false])` を `setSubmitted(true)` の手前で呼ぶ) — `useEffect` 内での setState は `react-hooks/set-state-in-effect` lint で禁止のため。
