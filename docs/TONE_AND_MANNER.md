...
| `--color-paper-shade` | `#f7eedd` | body 全体の縦グラデーション下端。paper より僅かに暖色。 |
...
2. **柔らかな敬体と体言止めを混ぜる**。「〜です」「〜ます」「〜ています」「〜ましょう」「〜てください」と、「〜タイプ。」「〜の日。」のような体言止めをリズム良く交互に置く。常体 (「〜だ」「〜である」) と過剰敬語 (「ございます」「いたします」) はどちらも避ける。
3. **命令形より勧奨形**。「〜してみて」「〜してみてください」「〜がお守りに」「〜してみましょう」を多用する。
4. **断定語は禁止**。「絶対」「必ず」「決して」「間違いなく」は使わない。代わりに「きっと」「やがて」「傾向」「印象」「サイン」「気配」「日和」で軟化。
...
| 2026-05-10 | §7 文体ルールを実データに合わせて修正 (「敬語禁止」→「柔らかな敬体と体言止めの混合」)。`--color-paper-shade` を追加し body グラデの HEX 直書きを除去。 |
